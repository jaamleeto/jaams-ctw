...
public final class LangManager {
	...
	public LangManager(JaamsCtwMod plugin) {
		this.plugin = plugin;
		...
		if (langFile.exists()) {
			...
			if (langVersion < minVersion && (langFile.getName().equals("spanish.yml") || langFile.getName().equals("english.yml")) || langFile.getName().equals("italian.yml")) { // Texts must be updated.
	...
	public String getMurderText(Player player, Player killer, ItemStack is) {
		String ret = ChatColor.translateAlternateColorCodes('&', lang.getString("death-events.by-player.message"));
	...
	public String getNaturalDeathText(Player player, EntityDamageEvent.DamageCause cause) {
		String ret = ChatColor.translateAlternateColorCodes('&', lang.getString("death-events.natural.message"));
		ret = ret.replace("%KILLED%", player.getName());
		ret = ret.replace("%KILLED_COLOR%", plugin.pm.getChatColor(player) + "");
