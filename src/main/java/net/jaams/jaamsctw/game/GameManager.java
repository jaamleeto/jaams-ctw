...
public class GameManager {
	...
	public GameManager(JaamsCtwMod plugin) {
		this.plugin = plugin;
		games = new TreeMap<>();
		events = new Events();
	...
	private void controlPlayers() {
		for (Player player : plugin.getServer().getOnlinePlayers()) {
			if (plugin.pm.getTeamId(player) == null) {
				if (plugin.mm.isMap(player.getWorld()) && !player.hasPermission("ctw.admin")) {
					plugin.getLogger().log(Level.INFO, "Unexpected event: Player {0} has no team and was on {1}", new Object[]{player.getName(), player.getWorld().getName()});
					player.teleport(plugin.wm.getNextLobbySpawn());
				}
			} else {
				if (!plugin.rm.isInGame(player.getWorld())) {
					plugin.getLogger().log(Level.INFO, "Unexpected event: Player {0} has team and was on {1}", new Object[]{player.getName(), player.getWorld().getName()});
	...
	public void movePlayerTo(final Player player, TeamManager.TeamId teamId) {
		...
		if (roomName != null) {
			...
			if (teamId != TeamManager.TeamId.SPECTATOR && !plugin.hasPermission(player, "override-limit") && getPlayersIn(roomName) >= game.mapData.maxPlayers) {
	...
	public void playerLeftGame(Player player) {
		...
		plugin.lm.sendVerbatimTextToWorld(plugin.lm.getText("player-left-map").replace("%PLAYER%", plugin.pm.getChatColor(player) + player.getName()), player.getWorld(), player);
	...
	public Location getRedSpawn(String roomName) {
		...
		return new Location(game.world, game.mapData.redSpawn.getBlockX(), game.mapData.redSpawn.getBlockY(), game.mapData.redSpawn.getBlockZ(), game.mapData.redSpawn.getYaw(), game.mapData.redSpawn.getPitch());
	}
	...
	public Location getBlueSpawn(String roomName) {
		...
		return new Location(game.world, game.mapData.blueSpawn.getBlockX(), game.mapData.blueSpawn.getBlockY(), game.mapData.blueSpawn.getBlockZ(), game.mapData.blueSpawn.getYaw(), game.mapData.blueSpawn.getPitch());
	}
	...
	public Game addGame(String roomName) {
		...
		for (String color : game.mapData.redWoolWinPoints.keySet()) {
			...
			t.location = new Location(game.world, tempLoc.getBlockX(), tempLoc.getBlockY(), tempLoc.getBlockZ());
		...
		if (game.mapData.blueInaccessibleAreas != null) {
			for (Selection sel : game.mapData.blueInaccessibleAreas) {
				game.bluePhoibitedAreas.add(new CuboidSelection(game.world, sel.getNativeMinimumPoint(), sel.getNativeMaximumPoint()));
			}
		}
		if (game.mapData.redInaccessibleAreas != null) {
			for (Selection sel : game.mapData.redInaccessibleAreas) {
				game.redPhoibitedAreas.add(new CuboidSelection(game.world, sel.getNativeMinimumPoint(), sel.getNativeMaximumPoint()));
			}
		}
	...
	public void cancelProtectedChest(InventoryOpenEvent e) {
		...
		Game game = worldGame.get(player.getWorld());
		if (game != null && (e.getInventory().getHolder() instanceof Chest || e.getInventory().getHolder() instanceof DoubleChest)) {
			TeamManager.TeamId teamId = plugin.pm.getTeamId(player);
	...
	public void checkTarget(Player player, ItemStack is) {
		...
		if (game != null) {
			...
			if (is.getType() == Material.WOOL) {
				...
				final String message = plugin.lm.getText("wool-pickup-message").replace("%PLAYER%", plugin.pm.getChatColor(player) + player.getName()).replace("%WOOL%", Utils.toChatColor(wool.getColor()) + wool.getColor().name());
						...
						for (String colorName : game.mapData.blueWoolWinPoints.keySet()) {
							if (colorName.equals(wool.getColor().name())) {
								...
								for (Player allPlayers : game.world.getPlayers()) {
									TitleAPI.sendFullTitle(allPlayers, 10, 60, 10, "§6" + Utils.Chars.caution,
											plugin.lm.getTitleMessage("wool-pickup-message").replace("%PLAYER%", plugin.pm.getChatColor(player) + player.getName()).replace("%WOOL%", Utils.toChatColor(wool.getColor()) + wool.getColor().name()));
			...
			if (t != null) {
				if (e.getBlock().getType() == Material.WOOL) {
					...
					if (wool.getColor() == t.color && t.team == plugin.pm.getTeamId(e.getPlayer())) {
						...
						final String winText = plugin.lm.getText("win-wool-placed").replace("%PLAYER%", e.getPlayer().getName()).replace("%WOOL%", t.color.toString());
						plugin.lm.sendVerbatimTextToWorld(winText, e.getBlock().getWorld(), null);
						for (Player allPlayers : game.world.getPlayers()) {
							TitleAPI.sendFullTitle(allPlayers, 10, 60, 10, "§a" + Utils.Chars.yestitle, plugin.lm.getTitleMessage("win-wool-placed").replace("%PLAYER%", e.getPlayer().getName()).replace("%WOOL%", t.color.toString()));
						...
						Utils.firework(plugin, e.getBlock().getLocation(), wool.getColor().getColor(), wool.getColor().getColor(), wool.getColor().getColor(), FireworkEffect.Type.BALL_LARGE);
						updateScoreBoard(game);
						if (plugin.db != null) {
	...
	private void updateScoreBoard(Game game) {
		...
		for (Target target : game.targets.values()) {
			...
			String lineName = String.valueOf(state) + Utils.toChatColor(target.color) + " " + ChatColor.WHITE + target.color.toString();
	...
	private void checkForWin(Game game) {
		...
		if (redComplete) {
			...
			for (Player allPlayers : game.world.getPlayers()) {
				Utils.firework(plugin, allPlayers.getLocation(), Color.ORANGE, Color.RED, Color.FUCHSIA, FireworkEffect.Type.BALL_LARGE);
			...
			for (Player allPlayers : game.world.getPlayers()) {
				Utils.firework(plugin, allPlayers.getLocation(), Color.PURPLE, Color.TEAL, Color.BLUE, FireworkEffect.Type.BALL_LARGE);
				allPlayers.playSound(allPlayers.getLocation(), Sound.valueOf(plugin.getConfig().getString("sounds.team-win-sound")), 10.0F, 1);
			}
			if (!decorator.isEmpty()) {
				plugin.lm.sendVerbatimTextToWorld(ChatColor.GOLD + "" + decorator, game.world, null);
	...
	public void ajustWeather(WeatherChangeEvent e) {
		...
		if (game != null && game.mapData.weather.fixed && e.toWeatherState() != game.mapData.weather.storm)
		...
		for (Player player : game.world.getPlayers()) {
			if (plugin.getConfig().getBoolean("keep-teams-on-win")) {
				...
				if (plugin.getConfig().getBoolean("user-fireworks-on-win")) {
					Utils.firework(plugin, player.getLocation(), Color.GREEN, Color.RED, Color.BLUE, FireworkEffect.Type.BALL_LARGE);
				}
		...
		for (Game game : games.values()) {
			if (game.mapData.woolSpawners != null) {
				for (String woolColor : game.mapData.woolSpawners.keySet()) {
					...
					Location loc = new Location(game.world, game.mapData.woolSpawners.get(woolColor).getBlockX(), game.mapData.woolSpawners.get(woolColor).getBlockY(), game.mapData.woolSpawners.get(woolColor).getBlockZ());
					for (Player player : game.world.getPlayers()) {
						if (player.getLocation().distance(loc) <= 6 && !plugin.pm.isSpectator(player)) {
