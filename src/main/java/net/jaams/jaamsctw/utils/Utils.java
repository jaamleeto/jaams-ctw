...
public class Utils {
	...
	public static void firework(JaamsCtwMod plugin, Location loc, Color color1, Color color2, Color color3, FireworkEffect.Type type) {
		...
		new org.bukkit.scheduler.BukkitRunnable() {
			...
			public void run() {
				for (int i = -2; i < 3; i++) {
					...
					data.addEffects(org.bukkit.FireworkEffect.builder().withColor(color1).withColor(color2).withColor(color3).with(type).trail(new java.util.Random().nextBoolean()).flicker(new java.util.Random().nextBoolean()).build());
	...
	public static String toString(Selection sel) {
		...
		return "X:" + min.getBlockX() + ", Y:" + min.getBlockY() + ", Z:" + min.getBlockZ() + " -> X:" + max.getBlockX() + ", Y:" + max.getBlockY() + ", Z:" + max.getBlockZ();
	}
}
