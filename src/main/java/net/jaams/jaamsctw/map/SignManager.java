...
public final class SignManager {
	...
	public SignManager(JaamsCtwMod plugin) {
		this.plugin = plugin;
	...
	public void load() {
		...
		for (String roomName : signConfig.getKeys(false)) {
			...
			Location loc = new Location(world, signConfig.getInt(roomName + ".x"), signConfig.getInt(roomName + ".y"), signConfig.getInt(roomName + ".z"));
		...
		if (plugin.rm.exists(roomName)) {
			if (plugin.rm.isEnabled(roomName)) {
				...
				if (mapName != null && plugin.mm.exists(mapName)) {
					...
					nowPlaying = nowPlaying + ChatColor.BLACK + " / " + ChatColor.AQUA + maxPlayers;
