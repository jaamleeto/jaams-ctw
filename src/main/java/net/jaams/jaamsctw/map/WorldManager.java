...
public final class WorldManager {
	...
	public WorldManager(JaamsCtwMod plugin) {
		this.plugin = plugin;
	...
	public World loadWorld(String worldName) {
		...
		if (world == null) {
			...
			if (worldDir.exists() && worldDir.isDirectory()) {
				WorldCreator creator = WorldCreator.name(worldName).seed(0).environment(World.Environment.NORMAL);
	...
	public void addSpawnLocation(Location spawnPoint) {
		String key = spawnPoint.getBlockX() + "x" + spawnPoint.getBlockY() + "y" + spawnPoint.getBlockZ() + "z";
		...
		for (int n = 0; n < lobbySpawnLocations.size(); n++) {
			...
			if (loc.getBlockX() == spawnPoint.getBlockX() && loc.getBlockY() == spawnPoint.getBlockY() && loc.getBlockZ() == spawnPoint.getBlockZ()) {
	...
	private static void cloneRegion(JaamsCtwMod plugin, World source, World destination, Selection area) {
		...
		for (int X = min.getBlockX(); X <= max.getBlockX(); X++) {
			for (int Y = min.getBlockY(); Y <= max.getBlockY(); Y++) {
				for (int Z = min.getBlockZ(); Z <= max.getBlockZ(); Z++) {
					...
					if (src.getType() == Material.SIGN_POST || src.getType() == Material.WALL_SIGN) {
					...
					} else if (src.getType() == Material.CHEST || src.getType() == Material.TRAPPED_CHEST) {
						Bukkit.getScheduler().runTaskLater(plugin, new Runnable() {
							@Override
							public void run() {
