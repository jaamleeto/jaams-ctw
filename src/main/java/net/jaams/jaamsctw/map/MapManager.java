package net.jaams.jaamsctw.map;
...
public final class MapManager {
	...
	public MapManager(JaamsCtwMod plugin) {
		this.plugin = plugin;
	...
	private MapData loadMapData(String mapName) throws IOException, InvalidConfigurationException {
		...
		mapData.maxPlayers = localConfig.getInt("max-players", mapsConfig.getInt(mapName + ".max-players", MAX_PLAYERS_DEFAULT));
		...
		if (mapsConfig.isSet(mapName + ".blue-inaccessible-area")) {
			mapData.blueInaccessibleAreas = getSelectionList(mapsConfig.getConfigurationSection(mapName + ".blue-inaccessible-area"), world);
		}
		if (localConfig.isSet("blue-inaccessible-area")) {
			if (mapData.blueInaccessibleAreas == null) {
				mapData.blueInaccessibleAreas = getSelectionList(localConfig.getConfigurationSection("blue-inaccessible-area"), world);
			} else {
				mapData.blueInaccessibleAreas.addAll(getSelectionList(localConfig.getConfigurationSection("blue-inaccessible-area"), world));
			}
		}
		if (mapsConfig.isSet(mapName + ".red-inaccessible-area")) {
			mapData.redInaccessibleAreas = getSelectionList(mapsConfig.getConfigurationSection(mapName + ".red-inaccessible-area"), world);
		}
		if (localConfig.isSet("red-inaccessible-area")) {
			if (mapData.redInaccessibleAreas == null) {
				mapData.redInaccessibleAreas = getSelectionList(localConfig.getConfigurationSection("red-inaccessible-area"), world);
			} else {
				mapData.redInaccessibleAreas.addAll(getSelectionList(localConfig.getConfigurationSection("red-inaccessible-area"), world));
			}
		}
		if (mapsConfig.isSet(mapName + ".protected-area")) {
			mapData.protectedAreas = getSelectionList(mapsConfig.getConfigurationSection(mapName + ".protected-area"), world);
		}
		if (localConfig.isSet("protected-area")) {
			if (mapData.protectedAreas == null) {
				mapData.protectedAreas = getSelectionList(localConfig.getConfigurationSection("protected-area"), world);
			} else {
				mapData.protectedAreas.addAll(getSelectionList(localConfig.getConfigurationSection("protected-area"), world));
			}
		}
		if (mapsConfig.isSet(mapName + ".red-wool-win-point")) {
			mapData.redWoolWinPoints = getWoolLocations(mapsConfig.getConfigurationSection(mapName + ".red-wool-win-point"), world);
		}
		if (localConfig.isSet("red-wool-win-point")) {
			if (mapData.redWoolWinPoints == null) {
				mapData.redWoolWinPoints = getWoolLocations(localConfig.getConfigurationSection("red-wool-win-point"), world);
			} else {
				mapData.redWoolWinPoints.putAll(getWoolLocations(localConfig.getConfigurationSection("red-wool-win-point"), world));
			}
		}
		if (mapsConfig.isSet(mapName + ".blue-wool-win-point")) {
			mapData.blueWoolWinPoints = getWoolLocations(mapsConfig.getConfigurationSection(mapName + ".blue-wool-win-point"), world);
		}
		if (localConfig.isSet("blue-wool-win-point")) {
			if (mapData.blueWoolWinPoints == null) {
				mapData.blueWoolWinPoints = getWoolLocations(localConfig.getConfigurationSection("blue-wool-win-point"), world);
			} else {
				mapData.blueWoolWinPoints.putAll(getWoolLocations(localConfig.getConfigurationSection("blue-wool-win-point"), world));
			}
		}
		if (mapsConfig.isSet(mapName + ".wool-spawner")) {
			mapData.woolSpawners = getWoolLocations(mapsConfig.getConfigurationSection(mapName + ".wool-spawner"), world);
		}
		if (localConfig.isSet("wool-spawner")) {
			mapData.woolSpawners = getWoolLocations(localConfig.getConfigurationSection("wool-spawner"), world);
		}
		if (mapsConfig.isSet(mapName + ".restauration-area")) {
			mapData.restaurationArea = getCuboidSelection(mapsConfig.getConfigurationSection(mapName + ".restauration-area"), world);
		}
		if (localConfig.isSet("restauration-area")) {
			mapData.restaurationArea = getCuboidSelection(localConfig.getConfigurationSection("restauration-area"), world);
		}
		...
		if (localConfig.isSet("no-drop-on-break")) {
			...
			for (String materialName : localConfig.getStringList("no-drop-on-break")) {
				try {
					...
					plugin.getLogger().log(Level.WARNING, "Ignoring invalid material name for map {0}: {1}", new Object[]{mapName, materialName});
				}
			}
		}
		...
		mapData.kitArmour = localConfig.getBoolean("kit-armour", mapsConfig.getBoolean(mapName + ".kit-armour", false));
		return mapData;
	}
	...
	private static TreeMap<String, Location> getWoolLocations(ConfigurationSection cs, World world) {
		...
		for (String colorName : cs.getKeys(false)) {
			result.put(colorName, new Location(world, cs.getInt(colorName + ".location.x"), cs.getInt(colorName + ".location.y"), cs.getInt(colorName + ".location.z")));
		}
		return result;
	}
	...
	public void announceAreaBoundering(PlayerMoveEvent e) {
		...
		if (mapData.blueInaccessibleAreas != null) {
			for (Selection sel : mapData.blueInaccessibleAreas) {
				...
				if (!selContainsFrom && selContainsTo) {
					...
					plugin.lm.sendMessage(ChatColor.BLUE + direction + "zona inaccesible para Azules " + ChatColor.GREEN + "(" + Utils.toString(sel) + ")", player);
		...
		if (mapData.redInaccessibleAreas != null) {
			for (Selection sel : mapData.redInaccessibleAreas) {
				...
				if (!selContainsFrom && selContainsTo) {
					direction = ChatColor.GREEN + "Entrando ";
					plugin.lm.sendMessage(ChatColor.RED + direction + "zona inaccesible para Rojos " + ChatColor.GREEN + "(" + Utils.toString(sel) + ")", player);
				} else if (selContainsFrom && !selContainsTo) {
					direction = ChatColor.GREEN + "Saliendo ";
					plugin.lm.sendMessage(ChatColor.RED + direction + "zona inaccesible para Rojos " + ChatColor.GREEN + "(" + Utils.toString(sel) + ")", player);
				}
		...
		if (mapData.protectedAreas != null) {
			for (Selection sel : mapData.protectedAreas) {
				...
				if (!selContainsFrom && selContainsTo) {
					direction = ChatColor.GREEN + "Entrando ";
					plugin.lm.sendMessage(ChatColor.YELLOW + direction + "zona Protegida " + ChatColor.GREEN + "(" + Utils.toString(sel) + ")", player);
				} else if (selContainsFrom && !selContainsTo) {
					direction = ChatColor.GREEN + "Saliendo ";
					plugin.lm.sendMessage(ChatColor.YELLOW + direction + "zona Protegida " + ChatColor.GREEN + "(" + Utils.toString(sel) + ")", player);
				}
		...
		if (mapData.blueInaccessibleAreas != null) {
			for (Selection sel : mapData.blueInaccessibleAreas) {
				if (sel.contains(loc)) {
					...
					plugin.lm.sendMessage(ChatColor.GREEN + "Eliminado" + ChatColor.BLUE + "zona inaccesible para Azules " + ChatColor.GREEN + "(" + Utils.toString(sel) + ")", player);
					break;
				}
			}
		}
		if (mapData.redInaccessibleAreas != null) {
			for (Selection sel : mapData.redInaccessibleAreas) {
				if (sel.contains(loc)) {
					...
					plugin.lm.sendMessage(ChatColor.GREEN + "Eliminado" + ChatColor.RED + "zona inaccesible para Rojos " + ChatColor.GREEN + "(" + Utils.toString(sel) + ")", player);
					break;
				}
			}
		}
		if (mapData.protectedAreas != null) {
			for (Selection sel : mapData.protectedAreas) {
				if (sel.contains(loc)) {
					...
					plugin.lm.sendMessage(ChatColor.GREEN + "Eliminado" + ChatColor.YELLOW + "zona Protegida " + ChatColor.GREEN + "(" + Utils.toString(sel) + ")", player);
					break;
				}
			}
		}
