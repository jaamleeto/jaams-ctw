...
public final class RoomManager {
	...
	public void removeWools(String mapName, World newWorld) {
		for (Location loc : plugin.mm.getWoolWinLocations(mapName)) {
			Location capturePoint = new Location(newWorld, loc.getBlockX(), loc.getBlockY(), loc.getBlockZ());
			capturePoint.getBlock().setType(Material.AIR);
		}
	}

	public RoomManager(JaamsCtwMod plugin) {
		this.plugin = plugin;
	...
	public void swapMap(String roomName) {
		...
		plugin.wm.restoreMap(plugin.mm.getMapData(room.maps.get(room.mapIndex).getName()), room.worlds.get(room.mapIndex));
