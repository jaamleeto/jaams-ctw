package net.jaams.jaamsctw.commands;
...
public class CommandManager implements CommandExecutor {
	...
	public CommandManager(JaamsCtwMod plugin) {
		this.plugin = plugin;
					...
					case "listspawn" : {
						if (plugin.wm.getLobbySpawnLocations().isEmpty()) {
							...
							for (Location loc : plugin.wm.getLobbySpawnLocations()) {
								player.sendMessage(plugin.lm.getMessagePrefix() + " spawn #:" + pos + " X=" + loc.getBlockX() + ", Y=" + loc.getBlockY() + ", Z=" + loc.getBlockZ());
								pos++;
							}
						}
					}
				...
				switch (args[1].toLowerCase()) {
					case "add" :
						...
						if (plugin.mm.add(player.getWorld())) {
							player.sendMessage(plugin.lm.getMessage("map-successfully-added").replace("%MAP%", player.getWorld().getName()));
						} else {
							player.sendMessage(plugin.lm.getMessage("map-already-exists").replace("%MAP%", player.getWorld().getName()));
						}
					...
					case "copy" :
						if (args.length != 3) {
							...
							if (plugin.wm.cloneWorld(player.getWorld(), args[2]) != null) {
								...
								player.sendMessage(plugin.lm.getMessage("world-created").replace("%WORLD%", args[2]));
							}
						}
						break;
					default :
